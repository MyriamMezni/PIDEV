...
class EnfantController extends Controller
{
    ...
    public function ajoutAction(Request $request,$id){
        ...
        $fileS=fopen("C:/Users/ben younes/AppData/Local/Temp/".$request->get('image'),"r+");
        $fileSource=fread($fileS,filesize("C:/Users/ben younes/AppData/Local/Temp/".$request->get('image')));
        $extension=substr($request->get('image'),strrpos($request->get('image'),".")+1);
        $directory=$this->get('kernel')->getProjectDir()."\web\images\\";
        echo ($extension);
      ...
      //  $file->move($directory, $name);
        ...
        $fileS1=fopen("C:/Users/ben younes/AppData/Local/Temp/".$request->get('document'),"r+");
        $fileSource1=fread($fileS1,filesize("C:/Users/ben younes/AppData/Local/Temp/".$request->get('document')));
        $extension1=substr($request->get('document'),strrpos($request->get('document'),".")+1);
        $directory1=$this->get('kernel')->getProjectDir()."\web\images\\";
        echo ($extension1);
        $name1=rand(1, 99999).'.'.$extension1;
        $fileDestination1=fopen($directory1.$name1,"w+");
        fwrite($fileDestination1,$fileSource1);
        fclose($fileS1);
        fclose($fileDestination1);
        $enfant->setDocument($name1);
    ...
    public function afficheEnfantParParentAction($id){
        $tasks=$this->getDoctrine()->getManager()
            ->getRepository('PidevBundle:User')
            ->findAll();
        ...
        $normalizer->setCircularReferenceHandler(function ($id) {
            return $this->getDoctrine()->getManager()->getRepository('PidevBundle:Enfant')->AfficheEnfant($id);
        });
        $normalizers = array($normalizer);
        $serializer = new Serializer($normalizers);
    ...
    public function parsingNomEnfantDisplayAction($nom)
    {
        $users=$this->getDoctrine()->getRepository(Enfant::class)->AfficherNomEnfant($nom);
        $data=array();
        foreach ($users as $key=>$u)
        {
            ...
            $data[$key]['remarque']=$u->getRemarque();
            $data[$key]['image']=$u->getImage();
            $data[$key]['document']=$u->getDocument();
            $data[$key]['cantine']=$u->isCantine();
            $data[$key]['status']=$u->getStatus();
        }
        return new JsonResponse($data);
    }

    public function parsingNomEnfantParIdAction($id)
    {
        $users=$this->getDoctrine()->getRepository(Enfant::class)->AfficherNomSelonIdEnfant($id);
        $data=array();
        foreach ($users as $key=>$u)
        {
            $data[$key]['idEnfant']=$u->getIdEnfant();
