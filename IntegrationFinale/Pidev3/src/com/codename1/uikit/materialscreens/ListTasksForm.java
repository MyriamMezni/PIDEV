    ...
    public ListTasksForm(Resources res,User u) throws IOException {
      ...
      //  Image profilePic = res.getImage("user-picture.jpg");        
        ...
        setTitle("Affiche Enfant");
         ...
         bt_chercher.setUIID("LoginButton");
       //  bt_chercher=new Button("mail");
       ...
       // sp.setText(ServiceEnfant.getInstance().getListReclamations().get(ServiceEnfant.getInstance().getListReclamations().size()-1).getImage());
        add(sp);
    //   ArrayList<En> ServiceEnfant.getInstance().getEnfantsPourParent(Statics.CONNECTED_USER.getIdUser());
        ServiceEnfant serviceDemande =new ServiceEnfant();
        ArrayList<Enfant> allEnfants=serviceDemande.getInstance().getEnfantsPourParent(Statics.CONNECTED_USER.getIdUser());
    
        String i = allEnfants.get(allEnfants.size()-1).getImage();
           bt_SMS= new Button("SMS");
          bt_SMS.setUIID("LoginButton");
             ImageViewer imgv= new ImageViewer();
                String urlMenu="http://localhost/PidevF2/pidev/web/Images/"+i;
                Image Imenu=URLImage.createToStorage(enc, urlMenu, urlMenu, URLImage.RESIZE_SCALE);
                imgv.setImage(Imenu);
        Container C2 = new Container(new BoxLayout(BoxLayout.Y_AXIS));
          Container C1 = new Container(new BoxLayout(BoxLayout.X_AXIS));
          
          Label l_nom = new Label(allEnfants.get(allEnfants.size()-1).getNom());
        Label l_prenom = new Label(allEnfants.get(allEnfants.size()-1).getPrenom());
       //  Date l_datenaissance = new Date(ServiceEnfant.getInstance().getListReclamations().get(ServiceEnfant.getInstance().getListReclamations().size()-1).getDatenaissance());
        Label l_remarque = new Label(allEnfants.get(allEnfants.size()-1).getRemarque());
        //  Label l_remarque = new Label(ServiceEnfant.getInstance().getListReclamations().get(ServiceEnfant.getInstance().getListReclamations().size()-1).getRemarque());
           Label l_cantine = new Label(Integer.toString(allEnfants.get(allEnfants.size()-1).getCantine()));
        ...
        //  String sppp=format(ServiceEnfant.getInstance().getListReclamations().get(0).getDatenaissance());
         //  C2.add(sppp);
